...
The company I work for makes microscope-like devices that monitor chemical processes, continually capturing and analyzing images to measure particle sizes. The application is written in C++ for the core logic, with the UI implemented in an embedded Chromium page.

Our UI shows images, charts, and tables of data, all dynamically generated by JavaScript. There's a ton of code, with each visual component requiring thousands of lines to build the DOM nodes, change styles, etc.

I don't want to criticize the person that wrote most of if because they wrote the bulk of it years ago and did an amazing amount of work. But I haven't enjoyed working on the UI. Examining the code to understand it, even a small part of it, requires scrolling all over the place. And I always find unexpected gotchas. For example, I'll change a function that enables or disables a button or a textbox and find that it doesn't work because the state of that component is also set in two to three other places, and some other module is overwriting my DOM update at a later time.
...
Solid is a client side framework where the HTML, CSS, and JavaScript are initially loaded by the browser and the UI is rendered and updated completely in the browser, unlike the traditional method where UI changes are rendered by refreshing the page from the server.
...
====Open https://jsfiddle.net/S0Eric/L9b2vdxk/9/ ====
...
To not run too long, I'll try to keep it moving, but please don't hesitate to ask questions if something doesn't make sense, or a term is unclear.

# JSX

For the templating syntax, Solid recommends JSX. It supports a couple of other templating languages, but JSX seems great to me.

JSX is a syntax extension to JavaScript, created for the React framework, but it can be used by any framework that supports it. It allows an HTML like syntax to be included in your JavaScript code for describing parts of your UI. It requires a compiler pass to convert the JSX syntax into JavaScript that creates the described DOM nodes.

An example Solid component won't make much sense without some familiarity with JSX. Should I cover the basics, or is everyone already familiar with JSX?
...
Without "React" support turned on, the JSX expressions makes for invalid JavaScript. JSX looks a lot like HTML to make it familiar to web developers.

Ok. On the left is our JavaScript with JSX expressions in it, and on the right is the resulting JavaScript code after compiling. By default, it generates code that invokes React functions to create the equivalent DOM nodes.
...
To show that it isn't just for React, let's add a directive to tell this JSX compiler to use a fictitious function to create the nodes.
...
This isn't exactly how Solid does it, but it makes the point that JSX isn't only tied to React.

The node creation function takes three or more arguments, the first being the element name, the second being an object with all the element's attributes, and the rest being any number of child elements. If a string is passed as the third argument, it becomes a child text mode. The resulting code can be hard to read when there is deep nesting of child nodes.

The function that creates these nodes often create DOM nodes, but it could create any kind of node. For example, it could create nodes in the shadow DOM. Solid doesn't use a shadow DOM, and if we have time at the end we can compare it to a shadow DOM approach.

There is a lot going on in these examples. I'm in the habit of using arrow functions instead of the old JavaScript function syntax, because they eliminate problems with "this" references that plague JavaScript. Should I quickly cover arrow function syntax?

JSX is typically used in functions, that are reusable UI components, like the OurButton component. See that OurButton is just a function that takes a props argument and returns a JSX expression, making it reusable as an element. See the Header component contains three OurButtons. Any properties passed become members of the props argument. In the case of OurButton, it only takes a single text property that becomes the child text of the button.

OurButton is written as a multiline function, with curly braces, so it requires a return statement, and could contain more lines of code before the return statement. But since it returns a single line of JSX, it can be simplified, as an arrow function that returns an expression:
...
Another strange thing here is the empty angle brackets. Because all JSX expressions result in a reference to a top level node, if you don't have a natural top level node, you enclose your list of nodes in what is called a fragment - these empty angle brackets. You can see that the function call on the right passes a special constant to indicate it is a fragment, and when this expression is inserted into a larger node tree, it will insert the children of the fragment. The fragment is just a container.

Notice how element attributes, called properties in JSX, are combined into an object and passed as the second argument to the node create function. For example, the resulting code shows the three references to OurButton in the Header component compile to code that passes the OurButton function as the first argument to the node creation function, and an object containing the specified properties as the second argument. During rendering, when the node create function is passed a function as the first argument, it calls the function and passes the properties object to that function as the first argument.

Besides basic HTML-like syntax and reusable component functions, the other powerful feature that JSX provides is that some parts of it, like property values and child text nodes can be specified as JavaScript expressions. These expressions have to be enclosed in curly braces. If the expression is a JavaScript object, double curly braces are required - the outer ones to specify that this is a JavaScript expression, and the inner ones to indicate the expression is an object. These expressions can be any valid JavaScript, a constant, variable, function call, etc.
